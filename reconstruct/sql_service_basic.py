...
def __get_conjunct_server_pred_structure(DNF_formula, attr2server_dict):
    ...
    for conjunct in DNF_formula:
        preds = [ pred.value for pred in conjunct ]
...
def compose_result(json_file_path, querysql):
    ...
    for parse_single in parse_ans:
        ...
        if (parse_single['type'] in ('SELECT','UPDATE') or
            ...
            executor = BasicExecutor(DNF, connection)
            cur_result = executor.execute(data, relation_table_name, unifed_sid_name)
...
result = compose_result('./table_config.json', test_query18)
print("\n\n##---------------\n")
print(result)
print("\n##---------------\n")
