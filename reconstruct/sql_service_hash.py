...
def compose_result(json_file_path, querysql, Executor, hash_structure, hash_function):
    ...
    for parse_single in parse_ans:
        ...
        if (parse_single['type'] in ('SELECT','UPDATE') or
            ...
            executor = Executor(data, relation_table_name, unifed_sid_name, DNF, connection, hash_structure, hash_function)
            cur_result = executor.execute()
...
meeting_host = '焦正材'  OR 
device_identifier = 'BE6C68AB011B267AF0F444FFFE3D4089BCB26549' AND meeting_time >= '2020-07-18 09:49:56' OR 
...
from executor_hash import BasicHashExecutor


flag_hash= False

result = compose_result('./table_config.json', test_query21, BasicHashExecutor, None, None)
print("\n\n##---------------\n")
print(result)
print("\n##---------------\n")
